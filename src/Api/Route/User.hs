...
import Api.Type (UpdateUserRequest (..), User (..))
...
data UserRoutes mode = UserRoutes
  { get :: mode :- Get '[JSON] User
  -- ^ GET /api/user
  , update :: mode :- ReqBody '[JSON] UpdateUserRequest :> Put '[JSON] User
  -- ^ PUT /api/user
  }
...
getCurrentUserHandler :: S.AuthResult UserId -> App User
...
updateCurrentUserHandler :: S.AuthResult UserId -> UpdateUserRequest -> App User
updateCurrentUserHandler (S.Authenticated uid) (UpdateUserRequest mEmail mUsername mPassword mBio mImage) = do
  ...
  case mUser of
    ...
    Just u -> do
      ...
      token <- liftIO $ generateToken jwtKey uid
      return $ User newUser.email token newUser.username newUser.bio newUser.image
